...
export const createAnothertrip = async(req, res)=>{
  ...
  file.mv(`./public/images/anothertrip/${fileName}`, async(err)=> {
    ...
    const {harga, keterangan_layanan, deskripsi_layanan, lokasi, keterangan_singkat} = req.body;
...
export const updateAnothertrip = async (req, res) => {
  try {
    ...
    const { nama_layanan, harga, keterangan_layanan, deskripsi_layanan, lokasi, keterangan_singkat } = req.body;
...
export const deleteAnothertrip = async(req, res)=>{
  ...
  if(!anotherTrip) return res.status(404).json({msg: "Data Destinasi tidak di temukan"});
